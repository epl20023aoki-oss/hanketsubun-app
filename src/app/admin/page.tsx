...
export default function AdminPage() {
  ...
  const [submittedReports, setSubmittedReports] =
    useState<any[]>([]);

  const [submittedTestimonies, setSubmittedTestimonies] =
    useState<any[]>([]);

  const [submittedSummaries, setSubmittedSummaries] =
    useState<any[]>([]);

  const [submittedTeamReports, setSubmittedTeamReports] =
    useState<any[]>([]);

  const [expandedTestimonies, setExpandedTestimonies] =
    useState<string[]>([]);
  ...
  return (
    ...
    >
      <div className="mx-auto max-w-4xl">
        ...
        {/* タブ */}
        <div
          className={`mb-6 flex flex-wrap rounded-2xl p-1 ${
            darkMode
              ? "bg-gray-800"
              : "bg-gray-100"
          }`}
                ...
                >
                  ...
                  >
                    {expandedTestimonies.includes(
                      ...
                      ? item.testimony
                      : item.testimony.length >
                        100
                      ? item.testimony.slice(
                          0,
                          100
                        ) + "..."
                      : item.testimony}
...
{activeTab === "teamReports" && (
  <div>
    ...
    >
      ...
      <span className="font-semibold">
        {
          submittedTeamReports.filter(
            (item) => {
              const matchName =
                !searchName ||
                item.leader?.includes(searchName) ||
                item.subLeader?.includes(searchName) ||
                item.members?.some(
                  (member: string) =>
                    member.includes(searchName)
                );
    ...
    {submittedTeamReports
      ...
      .map((item) => (
        ...
        <Link
          key={item.id}
          href={`/admin/team-reports/${item.id}`}
          className={`mb-3 block rounded-3xl border p-5 transition-all hover:scale-[1.01] ${
            darkMode
              ? "border-gray-700 bg-gray-800 hover:bg-gray-700"
              : "border-gray-100 bg-white hover:bg-gray-50"

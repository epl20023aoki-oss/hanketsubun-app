...
export default function TeamReportInputPage({ params }: Props) {
  ...
  useEffect(() => {
    const loadReport = async () => {
      try {
        ...
        const shareRef = doc(db, "team_report_inputs", shareId);
        const shareSnap = await getDoc(shareRef);

        if (!shareSnap.exists()) {
          alert("この共有入力URLは見つかりません");
          return;
        }
        ...
        setRoute(String(shareData.route || ""));
        setTeamName(shareData.team || "");
        setStartDate(shareData.startDate || "");
        setEndDate(shareData.endDate || "");

        const normalizedMembers: Member[] = [];

        if (shareData.leader) {
          normalizedMembers.push({
            name: shareData.leader,
            role: "班長",
          });
        }

        if (shareData.subLeader) {
          normalizedMembers.push({
            name: shareData.subLeader,
            role: "副班長",
          });
        }

        const rawMembers = Array.isArray(shareData.members)
          ? shareData.members
          : [];
        ...
        console.error("共有週間レポート読み込みエラー", error);
        alert("読み込みに失敗しました");
      } finally {
        setLoading(false);
      }
  ...
  const handleSelectName = async (
    ...
    try {
      ...
      if (!shareSnap.exists()) {
        alert("共有入力データが見つかりません");
        return;
      }

      const shareData = shareSnap.data();
      const input = shareData.inputs?.[name] || {};
